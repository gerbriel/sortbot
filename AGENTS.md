...
│   │   ├── HomeDashboard.tsx / .css  # THE 'home' view (§6) — the page a signed-in member lands on. Seven widgets in one auto-fit grid: current batch (counts + Resume + a two-step ConfirmAction "Start a new batch"), recent batches (opens through `getWorkflowBatch` → App's `handleOpenBatch`, because the metadata read projects `workflow_state` away), Inbox (consumes `useSupportThreads` — never subscribes), Tools (built FROM the `navItems` prop, so a role gate is declared once), Storage, Marketplaces, and a founder 7-day pulse. NOT wrapped in `<ToolView>` — there is nothing to go Back from on the page you arrive at; it restates ToolView's spacing scale instead. Every widget FAILS QUIET (a failed read renders its empty state) and a feature whose migration has not run hides entirely. **First real adopter of `src/components/ui/`** (StatTile/StatGrid, EmptyState, Button, ConfirmAction, Badge, Skeleton) — no primitive was edited.
...
│   │   ├── home.ts / .test.ts     # PURE derivation for the home dashboard (23 tests): `batchSummary` (photos / true multi-photo groups / listings **via the shared `buildGroupArray`**, so the number cannot disagree with the step it links to / categorized), `recentBatchRows` (skips the OPEN batch, keeps server `updated_at DESC` order, clamps `current_step` into 1-4, and treats the literal string `"null"` as no name — §14 #6), `publicationCounts` (per status, plus PLACEMENTS vs DISTINCT listings, which differ the moment a shop cross-lists), `storageReadout` (bar clamped at 100%, `nearLimit` read off the RAW ratio so a 310% bucket still says "nearly full").
...
│   │   ├── productSearchService.ts # THE cross-batch listing search: `escapeIlikeTerm` (quote-string escaping + wildcard stripping — the or() injection guard), `buildSearchFilter`, `searchProducts` (ONE projected read, RLS-scoped, row-limited with a `truncated` flag), the PURE `groupRowsIntoListings` (products rows → listings, leader-tolerant, leader-first photos, price 0 → null), `fetchListing`, and the module-cached `fetchBatchNames`/`batchLabel`. 30 tests.
│   │   ├── labelsService.ts       # `listing_labels` / `product_labels` CRUD + assign/unassign, LABEL_COLORS (a palette NAME per row — the DB never stores a hex, so nothing from it reaches a style attribute), labelSwatch/sortLabels, `ensureSkus` (assigns missing SKUs, retrying on 23505 with a budget of 6 — uniqueness is the database's job, not a SELECT-first race) `findProductBySku`, `setProductCodes` (the SKU/barcode writer that reports 23505 in words rather than swallowing it, unlike `updateProduct`) and `countLabelUsage` (paginated, counts distinct LISTINGS not photo rows). `'unavailable'` pre-migration.
...
│   ├── seed-marketplace-vocab.ts     # Generates supabase/seeds/marketplace_vocab.sql from the adapters' colour/condition lists (`npm run seed:vocab`). Built by scripts/vite.seed.config.ts with scripts/stubs/ aliased over the logger + Supabase client.
├── supabase/seeds/                # GENERATED SQL run by hand, like the migrations: marketplace_vocab.sql (global vocabulary; on conflict do nothing).
...
|---|---|---|---|
| **Home (the landing page)** | `activeView === 'home'` — **any signed-in user; the INITIAL view** | `<HomeDashboard />` — deliberately NOT inside `<ToolView>` (there is nothing to go Back from on the page you arrive at) | `React.lazy` (own chunk, ~14 kB) |
...
| Products | `activeView === 'products'` — **any signed-in user, every workspace** | `<ToolView wide>` → `<ProductsView />` | `React.lazy` (own chunk, ~22 kB) |
...
and role gate. Below 640px a bottom tab bar adds Home / Workflow / Messages,
and its More tab opens that same menu. The header itself is a wordmark plus that
one trigger at every width.

- **`'home'` is where a signed-in session starts** (Sept 2026): `useState<ActiveView>('home')`, and both sign-out resets land there too, so the next sign-in on a shared machine does not open mid-workflow. It is first in the Work group of `navItems`, and **the header wordmark is a button that opens it** (`.app-wordmark-link` — a `<button>`, not an `<a>`: there is no router and a hrefless anchor is not focusable; it sets its own literal light colour and a WHITE focus ring, because the nav is the one inverted surface and `--accent` is black).
- **Library gave up its phone tab to Home.** `TAB_IDS` in `MobileNav.tsx` is now `home / workflow / messages`; the Library is one tap away from Home as both a widget and a menu row, whereas Home would otherwise be reachable only through "More".
- **Labels, Scan and Products are NOT founder-gated** — declared in `navItems` beside Library; Labels and Scan act on the open batch, Products acts on the whole workspace, and all three render `&& user` only. Labels and Products take `wide`.
- **Products is in `navItems` but NOT on the Home dashboard's Tools grid.** `HomeDashboard`'s `quickActions` is built from `navItems` through an explicit `ORDER` array, and that array names its tiles one by one — a new nav row therefore does NOT appear there until `'products'` is added to it. It was left out on purpose in this pass (HomeDashboard was off-limits); adding it is a one-word change to that array.
...
#### `support_thread_members` + two columns on `support_threads` (`supabase/migrations/team_messaging.sql` — WRITTEN, NOT YET RUN)

The second KIND of conversation. `support_threads` / `support_messages` (from `support_messaging.sql`) now carry both: `kind = 'support'` is the original one-user-to-the-founders' inbox, unchanged and still the default for every existing row; `kind = 'team'` is a direct message between people in ONE workspace.

| Table | Column | Type | Notes |
|---|---|---|---|
| `support_threads` | `kind` | TEXT | `support` \| `team`, **default `support`**, CHECK on the pair — so the migration needs no backfill and no existing row changes meaning. |
| | `last_sender_id` | UUID | Who wrote the last message, maintained by the `support_after_message` trigger and NOT in the client UPDATE grant. It is the half of the team unread test that `last_sender_role` cannot answer: a support thread has two sides, a team thread has N. |
| `support_thread_members` | `(thread_id, user_id)` | UUID, UUID | **The PK IS the "in a conversation at most once" rule** — a double-add hits 23505 rather than creating a second row (same shape as `product_labels`). Both FKs cascade. |
| | `email` | TEXT | Denormalised, because `auth.users` is not client-readable and the thread list has to print who is in a conversation. The INSERT policy proves it against `org_members` (case-insensitively), so it cannot be a made-up name. |
| | `last_read_at` | TIMESTAMPTZ | **My read marker.** Defaults to `now()`, which is what makes somebody added later see only messages sent AFTER they joined. The only column in the UPDATE grant. |
| | `added_at` | TIMESTAMPTZ | Not in the INSERT grant — the client supplies only who is in the conversation. |

**The creator has a row too**, deliberately: membership is then ONE rule with no special case, in the policies and in the client. **Founders of other workspaces cannot see a team thread** — the `is_beta_admin()` branch of every policy is gated on `kind = 'support'`, and a founder reaches a team thread only as a participant of their own workspace. `is_beta_admin()` is a cross-tenant power, and private colleague-to-colleague messages are not a thing this product can promise to read.

**Why a helper function and not a sub-select:** `support_threads_select` must ask "am I a participant?" and `support_thread_members_select` must ask "is this a thread I belong to?", which as plain sub-selects reference each other and raise `42P17 infinite recursion detected in policy`. `app_private.is_thread_participant(uuid)` (SECURITY DEFINER, `public` SECURITY INVOKER wrapper, §18 #45) reads the members table as the table owner, so no policy is evaluated and the cycle is cut at the one point where it can be cut without writing the rule twice.

`support_threads` also gains **DELETE**, scoped to `kind = 'team' and user_id = auth.uid()`. It exists for exactly one caller: `createTeamThread` writes the thread, then the roster, then the first message, and a failure at step 2 or 3 must not leave a half-built conversation in the creator's list forever. Support threads stay undeletable from any client.

#### Marketplaces (`supabase/migrations/marketplaces.sql` — WRITTEN, NOT YET RUN)

Where a workspace sells, what each marketplace calls things, which batch is aimed at which, and what has actually been published where. `marketplace` in all three tables is `MarketplaceKey` from `src/lib/marketplaces/types.ts` — the ten keys are a CHECK, and that file says **never rename**.

| Table | Column | Type | Notes |
|---|---|---|---|
| `org_marketplaces` | `org_id`, `marketplace` | UUID, TEXT | **The PK is the rule** — a workspace enables a marketplace at most once, so there is no surrogate id and no unique index to drift from it. `org_id` carries **no DEFAULT**, the one departure from the `default_org_id()` convention: it is half the conflict target, so an upsert needs it. Safety is `is_org_admin(org_id)` as both USING and WITH CHECK, which is stronger — `default_org_id()` picks *a* workspace, this proves the caller administers *that* one. |
| | `enabled` | BOOL | Default true. Off keeps the row (and its settings) but takes the marketplace out of Step 4. |
| | `settings` | JSONB | `{ pricingRuleId?, defaultCondition?, shippingProfile?, notes? }`, CHECKed only as an object. **`pricingRuleId` POINTS AT** a rule id in `description_settings.platformPricing` — the numbers are never copied, so one markup has one place to edit it, and an id that no longer resolves shows a `--warning` line rather than silently exporting at list price. `normalizeMarketplaceSettings()` runs on every read AND every write. |
| | `created_by` / `created_at` | UUID / TIMESTAMPTZ | Outside the UPDATE grant → immutable. The PK columns ARE inside it, and only because PostgREST compiles an upsert to `on conflict … do update set <every body column>`; both halves of the UPDATE policy are `is_org_admin`, so a row can only move between two workspaces the same person administers. |
| `marketplace_vocab` | `org_id` | UUID **nullable** | **NULL = a GLOBAL row every workspace reads** (founder-curated, `is_beta_admin()` to write, exactly like `vocab_tables.sql`); set = that workspace's override, which **wins**. No DEFAULT — a default would make "global" unreachable by omission. Outside the UPDATE grant, so an override can never be promoted to a global row by an edit. |
| | `kind` | TEXT | `brand` \| `color` \| `condition` \| `size` \| `category` — `VocabKind` in the contract. |
| | `canonical` → `marketplace_value` | TEXT | What the app holds → what their picker calls it ("Ecko Unltd" → "Ecko Unlimited"). Unique per `(coalesce(org_id, all-zero uuid), marketplace, kind, lower(btrim(canonical)))` via an expression INDEX — the `coalesce` is what makes the global rows compare at all, since NULL is not equal to NULL, and the all-zero uuid can never be a real `organizations.id`. PostgREST cannot address an expression index with `onConflict`, so the service does find-then-update. A CHECK requires `canonical` to be **already trimmed** by `marketplace_vocab_trim()` (the whole Unicode whitespace set, not `btrim/1`'s U+0020) for the same reason `brand_aliases.heard` is: a tab-padded row would be indexed and permanently unreachable by the client. It does NOT lower-case — `canonical` is display text; case-insensitive uniqueness is the index's job. |
| | `created_by` / `created_by_email` | UUID / TEXT | Pinned on INSERT, immutable by column grant. **Any MEMBER writes a workspace row** — no admin gate, same reasoning as `brand_aliases.sql`: it is one shop's record of its own inventory, and the person who hits "remember this mapping" is whoever is dictating. |
| `workflow_batches` | `target_marketplaces` | TEXT[] | `not null default '{}'` + a `<@` CHECK against the ten keys. Chosen in Step 4 from the workspace's enabled set and remembered on the batch. An array, not a join table: read on every batch open, never queried across batches, at most ten elements. **No new RLS** — the existing policies cover members and that table has no column grants. |
| `listing_publications` | `(org_id, product_group_id, marketplace)` | — | Unique — that triple IS the matrix cell. |
| | `batch_id` | UUID | FK → workflow_batches **ON DELETE SET NULL**: deleting a batch must not erase the proof its listings were published. |
| | `product_group_id` | UUID | The group leader's id, **deliberately NOT a foreign key** — the leader's `products` row can be deleted or re-created while the listing stays live, and a cascade would destroy the only record it was ever posted. `sku` is denormalised at write time for the same reason, and so the matrix renders without a join. |
| | `status` | TEXT | `draft` \| `exported` \| `posted` \| `live` \| `sold` \| `removed`. A RECORD, not a derivation: removing a marketplace from a batch's targets stops offering it, never erases what happened. |
| | `org_id` | UUID | `default default_org_id()` here (there is no composite key forcing its hand), so the client never sends it; `upsertPublication` is find-then-update precisely so it never has to. Any member writes — the row is created by exporting, copying a pack or publishing. |

Three functions, all placed by §18 #43–#45: `my_email()` (re-created byte for byte, SECURITY INVOKER, not one of the eight wrapped helpers), `marketplace_vocab_trim(text)` (INVOKER, immutable, `authenticated` — a CHECK runs as the invoking role), and `marketplace_touch_updated_at()` (trigger, EXECUTE revoked from PUBLIC/anon/authenticated). Every policy helper call is `(select public.…())` per §18 #47. Rationale, RLS decisions and the 33-scenario verification transcript: `docs/marketplaces/02-data.md`.
...
- **Messaging** — `support_threads` / `support_messages` / `support_thread_members`. **TWO KINDS of conversation on the same tables** (`team_messaging.sql`): `kind = 'support'` is one user ↔ the founders' shared inbox (roles `user` / `founder`, Founding admins see every one), and `kind = 'team'` is a direct message between people in ONE workspace — participants enumerated in `support_thread_members` (the creator included), everybody posts as `member`, and **a founder of another workspace cannot see it**: the `is_beta_admin()` branch of every policy is gated on `kind = 'support'`. Unread is therefore two rules: support is "the other SIDE wrote last, after my stamp" (`user_last_read_at` / `founder_last_read_at`); team is "the last message is **not mine** and is newer than my stamp" (`last_sender_id` + my own `support_thread_members.last_read_at`), because a conversation with four people in it has no "other side" to name. The floating `SupportWidget` shows **support threads only** (`supportUnreadCount`); the full-page `MessagesView` shows both, with an All/Support/Team chip row and one inline compose panel that starts either kind (teammates come from `fetchOrgMembers` on the caller's own workspace). `createTeamThread` is three writes — thread, roster, first message — and **deletes the thread if either of the last two fails**, which is what `support_threads_delete` is for. Pre-migration the team half hides itself: `fetchThreads` retries with the old column list on `42703` / `PGRST200` / `PGRST204` / `PGRST100` and reports `teamAvailable: false`. One thread = one user; Founding admins see all SUPPORT threads. `sender_role` is enforced by RLS (a devtools user cannot post as 'founder'). A SECURITY DEFINER trigger maintains last_message_*, reopens closed threads on a new message, and stamps the sender's own read marker. Since Sept 2026 messaging has **two front ends onto one data source**: the floating `SupportWidget` (mounted for every signed-in user, including the waitlist gate) and the full-page `MessagesView` (`activeView === 'messages'`, reached from a header **Messages/Inbox** button with an unread badge). Both — and the badge — read `lib/supportStore`, which owns the single Realtime subscription and the single 45 s poll and reconciles optimistic writes by mirroring the `support_after_message` trigger. Unread = last message from the other side newer than my read stamp (`isUnread`). Known soft spot: the column grant lets a user write `founder_last_read_at` on their own thread — only affects the founders' unread badge for that thread.
...
### Products (Sept 2026, report: "pull individual products, and CRUD labels and barcodes")
**Files:** `ProductsView.tsx` / `.css`, `lib/productSearchService.ts`, `lib/labelsService.ts` (`setProductCodes`, `countLabelUsage`), App's `openProductInWorkflow` / `handleListingEdited`

A full-page view every member can open, and **the only surface in the app that is not batch-shaped.** Step 3 pages
through the open batch, Labels prints the open batch, the Library opens batches — so a reseller holding a garment
whose name they remember, but not which batch it was shot in, had nowhere to go.

**Finding.** One projected read on `products` (`searchProducts`), `.or()` across title / seo_title / sku / barcode /
vendor / product_category, `updated_at desc`, row-limited. **No `user_id` or `org_id` filter** — RLS scopes it
(§18 #1). An empty query is not an error: it returns the most recent work, so opening the view shows something.
`escapeIlikeTerm` handles TWO distinct hazards and they need different treatment: the `or()` grammar characters
(`,` `.` `(` `)` `:`) are neutralised by **double-quoting the value**, which keeps "Levi's 501, 32x34" matching the
title it names, while `\` and `"` are backslash-escaped so nothing can break out of those quotes; `*` and `%` are
**dropped**, because either one turns a search into "show me everything" (`_` is kept — it matches any single
character *including itself*, so a SKU containing one still finds itself).

**Rows are photos; the list shows listings.** `groupRowsIntoListings` is pure and tested: rows sharing a
`product_group` collapse, the leader is the row whose `id === product_group` (first row when that row is absent —
the legacy fresh-UUID case), fields are coalesced **leader-first** the way the CSV exporter coalesces a group, and
photos are position-ordered within a member with the leader's first. **A $0 or missing price becomes `null`, never
0** — 0 is what the export gate reads as "priced at nothing".

**THE ROW LIMIT IS ROWS, NOT LISTINGS,** and the view says so when it bites. A four-photo listing is four rows, so
60 rows can be 15 listings. Paginating instead would let the group reduction split a listing across a page
boundary, which is a worse failure than a visible cap.

**Editing.** Seven fields (title, brand, size, color, price, condition, category) go through
**`syncGroupFieldsToDatabase`** — the same writer Step 3 uses, so the leader row AND every member row carry the
value and no later regroup can strand it (§11). The patch is carried by minimal `{ id, productGroup, ...patch }`
stand-ins, because that function reads only those keys. The save reports into `saveStatusStore` and **reads the
boolean** rather than `.catch()`-ing it (§18 #41).

**It never writes the workflow store.** That would be a third writer racing Step 3's two debounced saves. It writes
the database and then tells App through `onListingEdited(batchId, groupId, patch)`; App patches the four store
arrays **only when that batch is the one open**, and triggers no auto-save (none of these fields is in the
`slimForWorkflowState` whitelist, so `workflow_state` has nothing to say about them).

**Barcodes.** SKU and free-text barcode are written by `setProductCodes`, which exists precisely because
`updateProduct` swallows the Postgres error — a duplicate SKU hits `products_org_sku_uidx` as a 23505, and *"That
SKU is already used"* is the only message a user can act on. **Generate SKU** is `ensureSkus([leaderId])`; the
leader is the row the scanner and the printed label read. A live Code 128 preview comes from `lib/barcode.ts`, and
**Print label** renders a one-label sheet using the same `LABEL_TEMPLATES` geometry and the same print technique as
`LabelPrintView` (everything invisible, this subtree painted back) — a separate `SingleLabelPrint` rather than an
edit to LabelPrintView, so the batch sheet is untouched.

**Labels.** `ListingLabelsPicker` is mounted with the listing's member ids, **memoized on the joined id string**
(it keys its fetch on that array). Under it, a **Manage labels** disclosure CRUDs the workspace vocabulary with the
existing `updateLabel` / `deleteLabel` behind the two-step `ConfirmAction` primitive (§18 #12), and
`countLabelUsage` says how many **listings** — not photo rows — a label is on, so a delete prompt states what it
will strip. The picker is remounted on a `labelsRev` counter after any vocabulary change, which is how it re-reads.
`.pv-card .llp-head { display: none }` hides the picker's own "Labels" head inside this card (the card has its own,
and the photo count it carried is already in the detail header) — scoped here rather than in the picker, because
Step 3 still needs it.

**Open in workflow.** `onOpenInWorkflow(productId, batchId)` → App's `openListingInStep3`, which now takes the batch
id: in the open batch it focuses as before; in another batch it routes to **`openProductInWorkflow`**, which awaits
`getWorkflowBatch` → `handleOpenBatch` and only then sets `focusListingId`, releasing it on a timer rather than the
next frame because PDG remounts (`key={currentBatchId}`) and its group array has to rebuild first. A listing with no
`batch_id` shows "Not in a batch" and offers no button. **`BarcodeScannerView` routes through the same function**
(its `ScannedProduct` already carried `batch_id`), which is what closed §14 #45.
...
45. ~~**The scanner cannot open a listing that is not in the OPEN batch.**~~ — **RESOLVED (Sept 2026, the Products pass).** The refusal was right only while the batch was UNKNOWN: jumping to Step 3 anyway parks the user on an unrelated listing. `ScannedProduct` already carried `batch_id`, so the scanner now hands it to `openListingInStep3(productId, batchId)`, which routes an other-batch hit through the new `openProductInWorkflow(batchId, productId)` — `getWorkflowBatch` → `handleOpenBatch` → focus, awaited in that order. The toast survives for the one case that genuinely has nowhere to go: a product with no `batch_id` at all. The Products view uses the identical path.
...
- ✅ **Team conversations — a second KIND of thread on the messaging tables (Sept 2026, `docs/reviews/19-team-messaging.md`)** — messaging was one shape: a user's line to the founders' shared inbox. It is now two, distinguished by `support_threads.kind`. `'support'` is unchanged down to the byte — every existing row defaults to it, the two read columns still mean what they meant, and the floating widget still shows exactly those threads. `'team'` is a direct message to one teammate or a few, private to its participants: the roster lives in the new `support_thread_members` (the creator has a row like everybody else, so membership is ONE rule), everyone posts as `member`, and **founders of other workspaces cannot see it** — the `is_beta_admin()` branch of every policy is gated on `kind = 'support'` on purpose, because a cross-tenant power that reaches private colleague-to-colleague messages is not a promise this product can keep. Three things were load-bearing. (1) **The policies are mutually recursive if written naively** — threads asks the members table who is in it, members asks the threads table which threads I am in, and Postgres answers `42P17`; `app_private.is_thread_participant(uuid)` (SECURITY DEFINER behind a `public` SECURITY INVOKER wrapper, created that way from the start so the hardening file never has to move it) reads the roster as the table owner and cuts the cycle at the only point where the rule is not duplicated. (2) **Unread needed a new column.** "The other side wrote last" is a two-sided test; with four people in a conversation there is no other side to name, so `last_sender_id` (trigger-maintained, outside the client UPDATE grant) plus my own `support_thread_members.last_read_at` replaces it, and the trigger stamps the SENDER's participant row for a `member` message instead of one of the two support columns. (3) **`perf_rls_initplan.sql` recreates exactly these five policies with their pre-team definitions** — and running it on top did not merely revoke team visibility, it INVERTED the guarantee: a participant lost their own thread while a founder in an unrelated workspace gained read AND write access to it, with nothing failing. Its section 8 now branches on whether `support_threads.kind` and the helper exist, and the two run orders were proven to produce **textually identical** policy sets. One more hazard was found the same way and is now spelled out at the top of the migration's ROLLBACK block: that block is a sequence of statements, not a transaction, and its last act (narrowing the role CHECKs) REFUSES while a `'member'` row survives — so a rollback started without first deleting the team threads stops half way, with the pre-team policies already restored and every surviving team conversation readable from the founders' inbox. Client: `createTeamThread` is three writes (thread → roster → first message) and **deletes the thread if either of the last two fails**, which is what the new team-only DELETE policy exists for; `MessagesView` gained an All/Support/Team chip row, a kind mark per row, participant emails as the row title, and one inline compose panel — not a modal, because ≤1024px the page is already a one-column list/conversation swap and a dialog would be a third layer to escape on a phone. Pre-migration the whole team half hides: `fetchThreads` retries with the old column list on `42703` / `PGRST200` / `PGRST204` / `PGRST100` and reports `teamAvailable: false`, so the code ships before the SQL. **1,137 → 1,157 tests green · build clean · lint 252 (the baseline).** The migration is verified on a throwaway Postgres and NOT run anywhere.
- ✅ **Phones: the corner buttons moved into the workspace menu (15 Sept 2026)** — below 640px `.ks-fab`
  (the shortcuts gear) and `.app-container .sw-root` (the Messages bubble) are `display: none`. The gear's
  panel is now optionally CONTROLLED (`open` / `onOpenChange` on `ShortcutsPanel`; App holds
  `shortcutsOpen`) and a phone-only `navItems` row `{ id: 'shortcuts', phoneOnly: true }` in the Setup
  group opens it — `handleNavSelect` treats that id as an action, not a view. `WorkspaceNavItem.phoneOnly`
  renders `.wsmenu-item--phone`, hidden at ≥641px. Messages needed no new row: the tab bar's Messages tab
...
- ✅ **Multi-marketplace phase 1 — the data and the workspace half (15 Sept 2026, `docs/marketplaces/02-data.md`)** —
  ...
  and the Workspace tab that drive them; the adapters and Step 4 are a separate pass that consumes this.
  **`supabase/migrations/marketplaces.sql` is WRITTEN AND NOT RUN.** Two decisions carry the design. (1) **Selling
  somewhere is a workspace decision; how it spells things is not.** `org_marketplaces` is the only ADMIN-gated table
  in the file — turning a marketplace on changes what every member is asked to publish to — while `marketplace_vocab`
  is member-written with no admin gate, verbatim the `brand_aliases.sql` argument: it is one shop's record of its own
  inventory, and the person who hits "remember this mapping" is whoever happens to be dictating. Its global rows
  (`org_id null`) are founder-curated and read by every tenant, like `vocab_tables.sql`, and a workspace row **wins**
  over one. (2) **The resolver never guesses.** `buildVocabResolver` is pure and synchronous (an adapter formats 375
  listings with zero round trips), folds `brand` through `normalizeBrand` so Step 3's spelling memory and this agree,
  folds only case and whitespace for the other kinds — `2X` and `2 X` stay different sizes — and returns **null** on
  no match, because a guess is exactly how a listing ends up under the marketplace's own default brand, which is the
  problem §2c exists to solve. Also here: `workflow_batches.target_marketplaces` (a `<@`-CHECKed `text[]`, no new RLS)
  and `listing_publications`, which is a RECORD and not a derivation — `batch_id` is `on delete set null` and
  `product_group_id` is deliberately not a foreign key, so deleting a batch or a leader row can never erase the proof
  that fourteen listings went live somewhere last month. `setBatchTargets` treats **0 rows updated as a failure**
  (§18 #41) and `upsertPublication` writes only the fields it was given, so a connector filling in `external_id`
  later cannot blank what the feed export wrote. **58 tests**; verified across **33 scenarios** on a throwaway
  Postgres 14 — cross-org isolation on all three tables, the admin gate both ways, the founder gate on global rows,
  case-insensitive uniqueness including the `coalesce(org_id, all-zero uuid)` trick that makes two NULL-org rows
  collide, the Unicode-trim CHECK (a TAB-padded value that `btrim/1` alone would have admitted), column-grant
  immutability of scope and authorship, PostgREST's upsert shape on the composite PK, the idempotent re-apply, the
...
- ✅ **Home — a dashboard to land on, and a way to put a batch down (Sept 2026, `docs/reviews/20-home-dashboard.md`)** — the app opened on Step 1 and stayed there, so Labels, Scan, the Library, the inbox, presets, categories, marketplaces and the founder tools were invisible unless you went looking in a menu, and the only way to release a batch was a `window.confirm` at the bottom of Step 4. `'home'` is now an `ActiveView` and the INITIAL one (§6). **The decision everything follows from: the workflow is parked behind `<main hidden>` exactly as it is behind every tool view**, so "exit batch" costs nothing — nothing unmounts, an upload in flight and Step 3's debounced saves keep running, and **Resume is a re-reveal, not a re-open** (no fetch, no `handleOpenBatch`). It is deliberately NOT a `<ToolView>`: every tool view leads with "Back to workflow", and a Back control on the page you arrive at would be a control that lies — it restates ToolView's spacing scale instead. `handleClearBatch` was split into `startNewBatch()` (the whole teardown, no dialog) and `handleClearBatch()` (`confirm()` + that), so Step 4 is byte-for-byte unchanged while the dashboard's **two-step `ConfirmAction`** respects Do Not #12. Seven widgets, each of which **fails quiet** — a failed read renders that widget's empty state, never an error wall, because this is the first screen after sign-in — and each of which HIDES ENTIRELY when its migration has not run (`hidden` is a real state, distinct from "loaded and empty"). Resume uses `resumeStep` from `phoneSteps.ts`, so it lands on the step the work is on rather than sending every reopened batch to Export; a recent-batch row costs one extra `getWorkflowBatch` read **on purpose**, because the metadata call projects `workflow_state` away and handing that to `handleOpenBatch` sends it down the DB-rebuild path; the open batch is filtered OUT of the list (it has its own widget) and its display NAME is read from the same response, so App needed no new state. Quick actions are built FROM `navItems`, so a role gate stays declared once. Navigation: a Home row first in the Work group, the header **wordmark is now a button that opens it**, and the phone tab bar is Home / Workflow / Messages / More (Library gave up its slot — it is one tap away from Home, whereas Home would otherwise live only behind "More"). **First real adoption of `src/components/ui/`** (§16): StatTile/StatGrid, EmptyState, Button, ConfirmAction, Badge, Skeleton, all used as-is — the one override is `.home-stats` narrowing StatGrid's 130px minimum track to 90px, documented at the rule, because every stat row here is exactly three tiles. **47 new tests (1572 → 1619, 73 → 75 files)**, build clean, lint flat at 252. Screenshots at 1280/820/390 caught three real defects, the third worth remembering: **`index.css` pins `white-space: nowrap` on every `button`**, so a label inside one inherits it and can never wrap — the tool tiles were clipping to "Manage Cate…" and a `-webkit-line-clamp` attempt made it worse (`display: -webkit-box` sizes its anonymous text child to max-content); `white-space: normal` on the label is the whole fix. Measured: `scrollWidth === clientWidth` at all three widths, zero sub-44px targets at 390. Still unverified: anything in a signed-in browser.

- ✅ **Products — pull one listing out of any batch, and CRUD its labels and barcodes (Sept 2026, `docs/reviews/21-products-view.md`)** — the founder asked to "pull individual products, and also CRUD labels and barcodes". Every surface in the app was batch-shaped, so a garment in the hand could only be reached by remembering which batch it was photographed in. `'products'` is now an `ActiveView` open to **every member** (§6), sitting beside Library in the Work group. Three decisions carry it. (1) **`escapeIlikeTerm` treats the two hazards differently**: the `or()` grammar characters are neutralised by double-QUOTING the value — so "Levi's 501, 32x34" still matches the title it names, where stripping them would have silently broken the search — while `\` and `"` are escaped so nothing breaks out of the quotes, and only `*`/`%` are dropped because either turns a search into "everything". (2) **The row limit is rows, not listings, and the view says so**: a four-photo listing is four rows, and paginating instead would let the pure `groupRowsIntoListings` reduction split a listing across a page boundary — a worse failure than a visible cap. (3) **It never writes the workflow store.** Edits go through `syncGroupFieldsToDatabase`, the same writer Step 3 uses (leader row + every member, §11), report into `saveStatusStore` by reading the returned BOOLEAN rather than `.catch()`-ing it, and then tell App via `onListingEdited` so App — the one owner of those arrays — mirrors the change, and only when that batch is the one open. A cleared price is written as `null`, never 0. `setProductCodes` was added beside `ensureSkus` for one reason: `updateProduct` swallows the Postgres error, and a duplicate SKU is a 23505 whose only actionable form is *"That SKU is already used"*. `countLabelUsage` counts distinct **listings**, not `product_labels` rows, so a delete prompt does not tell a founder that a four-photo jacket used a label four times. **Closed §14 #45 on the way**: `openProductInWorkflow` awaits `getWorkflowBatch` → `handleOpenBatch` → focus, and the scanner (which already carried `batch_id`) routes through it, so a code scanned off a rack whose batch was closed weeks ago now opens it. **51 new tests (1619 → 1670, 75 → 77 files)**, build clean, lint flat at 252. Screenshots at 1280/390 plus a print-media pass caught the two things a code read does not: the picker's own "Labels" head was the second one inside the card, and the truncation note wrapped on a phone into a dangling separator that read as a bullet. Measured: `scrollWidth === clientWidth` at 390, zero sub-44px controls, every control at 16px, and the print sheet measuring exactly 4in × 2in with nothing else visible. **Deliberately NOT done**: adding `'products'` to `HomeDashboard`'s explicit `quickActions` ORDER array (that file was out of scope, so the tile does not appear there yet — a one-word change), and a listing-level delete (destructive, and the Library already owns it). Still unverified: anything in a signed-in browser.
...
| **Multi-marketplace publishing** | **Phases 1+2 BUILT (migration not run); API connectors not started** | Plan: `docs/marketplaces/00-plan.md`. Built and shipped behind the unrun migration: the ten adapters (`01-adapters.md`), the data layer and the Workspace dashboard's Marketplaces tab (`02-data.md`), and Step 4's panel — targets, readiness matrix, feed CSVs, copy-ready packs, inline vocabulary fixes, publications (`03-step4.md`). **None of it needs an API key.** Run `marketplaces.sql` and it turns on; until then every read is `'unavailable'` and both surfaces show a setup hint. **Phase 3 — eBay, Etsy and Shopify WRITE — is not started and needs the founder's explicit yes on external APIs (plan §6.1).** Also not built: status sync-back and "sold on X → mark the rest" (phase 4), the seeded global vocabulary (`02-data.md` §7.1 — a `scripts/` one-off, not a migration), and the still-unverified `spec` numbers (`01-adapters.md` §3, every `verified` flag false). **Global vocabulary seed:** `npm run seed:vocab` regenerates `supabase/seeds/marketplace_vocab.sql` (859 global rows — colour names/aliases → each marketplace's fixed colour list, loose condition phrasings → each marketplace's condition word; brands deliberately NOT seeded) from the adapters' own lists via `scripts/seed-marketplace-vocab.ts`, built with `scripts/vite.seed.config.ts` (the logger and the Supabase client are aliased to `scripts/stubs/`). Run the SQL by hand after `marketplaces.sql`; `on conflict do nothing`, verified idempotent on a throwaway Postgres. |
