...
- **On phones the two floating corner buttons are gone.** The shortcuts gear is a "Shortcuts & debug" row in the workspace menu (the menu's Setup group; it opens the same full-screen panel), and the Messages bubble is replaced by the bottom bar's Messages tab and the menu's Messages / Inbox row. Desktop keeps both buttons. The waitlist screen keeps its bubble, since it has no menu.
