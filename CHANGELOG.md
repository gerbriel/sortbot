...
### Workspaces
- **Category and preset names are now unique per workspace, not across the whole app** (`supabase/migrations/tenant_scoped_uniqueness.sql`, to run). Until this runs, a second workspace cannot hold a category named "tees" because the Founding Workspace already does — which is why a newly approved shop opened Step 2 with no categories at all. Run it before onboarding anyone else; the seed then works.
- **C&D Vintage becomes its own workspace** (`supabase/oneoff/split_cd_vintage.sql`, to run after the migration above; runbook in `docs/reviews/24-cd-vintage-split.md`). CJ's gmail account becomes its owner and his batches, products, images, presets, categories, marketplaces, labels and Shopify connection go with him; his iCloud account stays in the Founding Workspace to see what a demo member sees. The Founding Workspace no longer exports under the "C&D Vintage" name — it uses its own, like every other workspace.

### Founder console
- **One page for running the beta.** Approving a shop, seeing every workspace and managing every account used to be three tabs inside the Workspace dashboard — a page that is otherwise entirely about the workspace you are signed in to. They now live in a **Founder console** (workspace menu → Founder, founding admins only), which also does three things that were not possible before: **onboard a workspace** before its owner has ever signed in (they become its owner if they already have an account, or get an admin invite if they do not, and their categories and presets are set up the first time they open it), **set a workspace's plan**, and **rename or invite into any workspace**. Expanding a workspace shows who is in it, its open invites, its counts and where it sells. An overview tab carries the pending-request count and a plain-English log of every change, with your name on it, and Home's founder widget now says how many requests are waiting. The Workspace dashboard keeps everything about your own workspace and one link across. Needs `supabase/migrations/founder_console.sql`; until it is run the three moved tabs work exactly as before and the new controls say what to run.

### Photos
- **Plain backgrounds, automatically.** A self-hosted service (ours — no background-removal SaaS, nothing generative) cuts each garment out of its photo and puts it on a flat backdrop at a fixed size, and stores that beside the original rather than over it. It scores its own work, so **you review the few it is unsure about, not all four hundred**: Step 2 gets a **Backgrounds ▾** button with the batch's counts and a Process action, a **Needs review** filter, and a small mark on the handful of cards that want a look — double-click one and the lightbox shows Before/After, says what it is unsure about in plain words ("cut off at an edge", "low contrast with the backdrop"), and offers Approve, Keep original, or Re-run at 2K. Everything downstream follows one rule: an approved photo exports its new background, and anything unsettled **blocks the export** the same way a $0 price does — the Shopify CSV and every marketplace feed, with a banner telling you where to go. A photo the service could not do, or that you chose to keep, exports untouched and blocks nothing. The backdrop, margin, size and quality are one setting per workspace (**Workspace → Settings → Photo backgrounds**, with a live preview); changing it only affects photos processed from then on, and Step 2 offers to re-apply it to older ones. Needs `supabase/migrations/image_backgrounds.sql`, the service deployed, and `VITE_MATTING_URL` set — miss any one and nothing appears, and your photos are handled exactly as they are today.

- **Photo backdrops, and failures that say why.** Backgrounds can now be one of your own surfaces — upload a few photo backdrops (a linen sheet, a wood floor; up to 8) in **Workspace → Settings → Photo backgrounds**, pick one, and the preview shows the real thing with a garment on it. Flat colour is one tap away, and removing a backdrop you are using puts the background back to the colour and tells you. When a photo cannot be processed the app now says **why** in the service's own words — on the card, in the lightbox and once under the counts in **Backgrounds ▾** when every failure has the same cause, with *"Add credit to the Replicate account, then Process again."* when that is what it is. And no photo gets stranded: if a run dies part-way, **Process** picks its photos back up and says how many it is including, and a job the service has forgotten ends with an instruction instead of a progress bar that never finishes.

### Pricing and research
- **The app now dates a piece, grades it, and suggests a price — with the reasoning next to it.** A new **Research** card in Step 3 reads what you already said out loud and shows an era, a condition with the flaws listed separately, a rarity, and a price — each with a confidence and a **Why** list you can open and argue with ("Heard 'nineties'", "Single stitch hem", "Champion was active in the 1990s"). When two things disagree it shows **both** sides and says so rather than picking one quietly. Nothing is filled in for you: every value has a **Use** button, and pressing it saves exactly as typing it would. **A price you said out loud always wins** — it is used as-is and nothing else is consulted. Otherwise the price is the median of **your own sold history** for that kind of piece, adjusted for condition and rounded, with every number in the explanation traceable to a real sale; asking prices are used only when there are fewer than three sales, are discounted 15%, and say so. When there is not enough to go on it says so and suggests nothing, rather than inventing a figure.
- **Step 4 splits the batch into "ready to export" and "needs a look"**, with each flagged listing's reason in words and a link straight back to it in Step 3 — a high-value listing priced on thin evidence, or one with no comparables at all. **It is advice, not a block**: the CSV still downloads, and every marketplace column shows the same flag as a warning rather than refusing the feed.
- **Mark as sold** (Products) records what a listing actually sold for, when, where and for which order — and shows its history with days to sell. That is the loop: every sale you record becomes a comparable the next similar piece is priced from, so the suggestions get better as the shop sells instead of staying flat. Every suggestion and every correction is written down from day one, because that record is the only part of this that cannot be recreated later.
- Needs `supabase/migrations/pricing_research.sql`; until it is run none of these surfaces appear and Steps 1-4 behave exactly as they do today. No external service, no API key, no third-party pricing data.
...
### Step 2 and Step 3
- **Step 2 piles.** Grouped photos are a stack: the first photo on top, the rest peeking behind with a `+N` badge; click (or Enter/Space) fans the stack open in place with drag-reorder, remove-from-group and a Collapse bar; Escape collapses. Singles are unchanged. A closed pile loads only its top photo (the peeking layers are blank card edges), so a batch of piles fetches one image per group until a pile is opened; Step 3 and the export are unaffected.
...
- **Step 2 View ▾ → Labels.** A "File names & dates" switch hides or shows the file name and capture time under each photo; off by default, and the choice is remembered on this device.
- **Step 3 progress bar simplified.** It shows only "Product Group N of M" over the green fill; the image-count and category chips and the "Select" checkbox are gone (the photos are right below, the category is in the form). Apply preset acts on the listing on screen; Paste Crop acts on every listing, as its label now says.
...
### Messaging
- **Message your workspace, not just us.** The Messages page now holds two kinds of conversation: the line to the Arcadian team, exactly as before, and **team threads** — a direct message to one teammate or a few, private to the people in it. New message lets you pick who it goes to (Arcadian support, or any teammates in your workspace); the list gains All / Support / Team filters, a mark on each row saying which it is, and the other participants' names as the row title. Founders see every **support** conversation as they always have, and team threads only in their own workspace, as an ordinary participant — **nobody at Arcadian can read another workspace's team messages**. The floating inbox button stays exactly what it was: conversations with us, and its unread count now counts only those. Unread on a team thread means the last message is not yours and you have not opened it since. Needs `supabase/migrations/team_messaging.sql` (then re-run `perf_rls_initplan.sql`); until it is run everything behaves as it does today and the team half hides itself.

### Home
- **A setup checklist on your first screen.** A new workspace lands on Home with **Get set up** at the top of the page: name your shop (it becomes the Vendor on every listing you export), review your categories and presets, choose where you sell, connect Shopify, invite your team, upload your first batch and export it. Each line says why it matters and takes you straight there. It ticks itself off as you go, counts the optional ones separately, and **disappears for good once the required steps are done** — or straight away, if you hide it. A teammate who is not an admin only sees the steps they can actually do, and anything your workspace has not set up yet is simply not on the list.
- **A home page to land on.** Signing in now opens a dashboard instead of dropping you straight into Step 1: the batch you have open (with its counts, the step it is on, **Resume**, and **Start a new batch**), your recent batches, your messages, a tile for every tool you can reach — Labels, Scan, Library, Categories, Presets, Workspace, Marketplaces, and the founder tools where they apply — your storage, and where this batch has been listed. **Leaving a batch costs nothing**: the workflow stays exactly where you left it, so an upload in progress keeps uploading and Resume puts you straight back on the step you were working on. The wordmark in the top-left is the way home from anywhere, and on a phone Home is the first tab (the Library moved into the menu and onto the dashboard itself). Widgets for features your workspace has not set up simply do not appear.

### Navigation
- **One step at a time.** The four workflow steps no longer sit on one long page at any screen size: a stepper (Upload · Group · Describe · Export) shows which step you are on and only that step is on screen. Click or tap any step you have reached to move forward or back, or use the Back and "Continue to …" buttons at the bottom of each one; steps you have not unlocked yet are greyed out and say what unlocks them. Uploading photos carries you to Group automatically; nothing else moves you on its own. Reopening a batch picks up on the step with work left, never on Export. Nothing is unmounted behind the scenes, so an upload in flight, your photo selection and Step 3's auto-save all keep running while you move between steps. On a desktop the Step 2 grid now uses the full height of the window, and adding photos to an active batch means stepping back to Upload.
- **Grouping actions moved into the Step 2 toolbar.** Group N, Ungroup and Clear lead the photo-tools row that appears with a selection (Delete N was already there), and Ungroup all sits at the right end of the idle row while any group exists. The action grid under the category panel and the dashed "drag photos here to make them individual items" strip are gone; ungroup with the button, the group's menu, or the shortcut.
- **Category panel tidied.** The "Drop Here to Categorize" heading is gone (the panel's own description already says it) and the category search box now sits above the Men / Women / Kids toggles. The five stacked action buttons below the categories are now a 2×2 grid (Group N · Ungroup · Delete N · Clear) that appears only while photos are selected, with one line of hint text when nothing is, and Ungroup all as a small link.
...
- **On phones the two floating corner buttons are gone.** The shortcuts gear is a "Keyboard shortcuts" row in the workspace menu (the menu's Setup group; it opens the same full-screen panel), and the Messages bubble is replaced by the bottom bar's Messages tab and the menu's Messages / Inbox row. Desktop keeps both buttons. The waitlist screen keeps its bubble, since it has no menu.
- **Keyboard shortcuts control only on Steps 2 and 3.** The gear (desktop) and the menu row (phones) appear only where the shortcuts apply; Home, the tool views, Step 1 and Step 4 no longer show them.
- **The debug-logging switch is gone from the shortcuts panel.** It was a developer control in product UI; the logger still exists and is turned on from the browser console via the `sortbot_debug_enabled` storage key.
- **Global marketplace vocabulary seed.** `npm run seed:vocab` generates `supabase/seeds/marketplace_vocab.sql` from the adapters' own lists: every colour name and alias the app knows mapped to Poshmark's and Vinted's fixed colour lists, and loose condition phrasings ("beat up", "barely worn", "sealed"…) mapped to each marketplace's condition word. Brands are not seeded, since no marketplace publishes its picker. Run the SQL once after the marketplaces migration.
...
- **Choose where you sell, and teach each place how you spell things.** The Workspace dashboard has a new **Marketplaces** tab. Turn on the marketplaces this shop actually lists on — Shopify, eBay, Etsy, Poshmark, Mercari, Grailed, Depop, Facebook, Vinted, Whatnot — and give each one a price rule (from the markups you already set under Settings) and a default condition; only the ones you turn on will ever be offered when a batch is exported. Below that, **Marketplace vocabulary**: Poshmark, Depop, Grailed, Vinted and Mercari pick a brand and a colour from their own list, so if we send a spelling they do not know they either reject the field or quietly choose one for you — teach them once here ("Ecko Unltd" → "Ecko Unlimited", "Forest Green" → "Green") and every listing goes out right. Anyone on the team can add a mapping; only an admin turns a marketplace on or off. Each batch will remember which marketplaces it is aimed at, and the app keeps a record of what was published where, so "sold on one, still listed on three" stays true. Needs `supabase/migrations/marketplaces.sql`; until it is run nothing changes — the tab shows a setup note and Step 4 exports exactly as it does today.
- **Step 4 now exports to all of them.** A **Marketplaces** panel sits at the top of Step 4. Pick which of your marketplaces this batch is going to — the chips are remembered on the batch — and a grid shows every listing against every one of them: a tick when it is ready, an amber count when something is worth fixing, a red count when it cannot go out (no price, no photo). Under each marketplace is the list of what is wrong, with a **Remember: «Nike» → [what Poshmark calls it]** box beside anything a spelling would fix; save it once and the grid clears it for every listing that shared it. For the marketplaces that take a file (Facebook, Depop, Whatnot) there is a **Download CSV** button, disabled until nothing is blocked, and each listing is recorded as exported. For the ones that do not (eBay, Etsy, Poshmark, Mercari, Grailed, Vinted) there are **listing packs**: the title, description, tags and every attribute already cut to that marketplace's limits, one tap to copy each or the whole thing, the photos in order as a zip, and a **Mark posted** button so the record stays true. **Shopify's CSV has not moved** — it is still the Export Options block below, which also checks your titles against the live store. Needs the same migration; until it is run Step 4 is exactly as it was.
