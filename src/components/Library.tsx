...
// ProductGroup and ImageRecord moved to lib/libraryData.ts (imported above).
...
export const Library: React.FC<LibraryProps> = ({ userId, onClose, onOpenBatch, onBatchDeleted, refreshTrigger, currentBatchId }) => {
  ...
  const loadAll = async (cancelRef?: { current: boolean }, force = false) => {
    ...
    try {
      ...
      // Pure derivation — extracted verbatim to lib/libraryData.ts (tested there).
      const { batches: finalBatchesDerived, groups: finalGroupsDerived, images: imageList } =
        deriveLibraryData(wfBatches, savedProducts, savedImages);

      // ── 5. Commit all state at once ─────────────────────────────────────
      if (!isCancelled()) {
        const finalBatches = finalBatchesDerived;
        const finalGroups = finalGroupsDerived;
