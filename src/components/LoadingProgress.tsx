  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 8 }}>
        <span style={{ color: 'var(--text-primary)', fontWeight: 600, fontSize: 'var(--fs-sm)' }}>{message}</span>
        {/* Was #6366f1 — 4.2:1 on this panel, under the bar at 0.88rem (~7.9px
            at the 9px root, so no large-text exemption). --accent is 6.8:1. */}
        <span style={{ color: 'var(--accent)', fontWeight: 700, fontSize: 'var(--fs-sm)', flexShrink: 0 }}>
          {Math.round(progress)}%
        </span>
      </div>
