...
- ✅ Step 2 capture date labels — when sort order is **↑ Date** or **↓ Date** (`sortOrder.startsWith('date')`), each single-item card shows its own `capturedAt` as a compact footer label (e.g. `Mar 12, 2024`); product group cards show the **earliest** `capturedAt` among all items in the group (via `Math.min` over members); labels are hidden when sorting by name; rendered as `.capture-date-label` div after `.item-info` in single-item cards and between `.group-header` and `.group-images` in group cards; styled with `font-size: 0.65rem`, grey `#888` text, `#f8f8f8` background, `border-top: 1px solid #eee` separator; group logic uses an IIFE to compute earliest date inline; `ImageGrouper.css` updated with the new class (commit `6646d47`)
