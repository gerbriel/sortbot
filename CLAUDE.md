...
- ✅ **Compress All Batches button v2 — storage bucket walk** (commit `9d9f1d5`) — fixed root cause by adding a second source: `supabase.storage.list(userId, { limit: 10000 })` gets all product subfolder UUIDs (~2,425); for each subfolder (in chunks of 50) `supabase.storage.list(`${userId}/${folder.name}`, { limit: 1000 })` lists the files inside; CDN URL built via `supabase.storage.getPublicUrl(storagePath)` for each file; results merged into a `Map<storagePath, url>` (DB entries overwrite walk-derived URL where both exist); **scanning phase UI sentinel:** `total === -1` means "still scanning" — UI shows `Scanning storage… N folders checked` during the walk; after scan completes, `total` is set to the actual needsWork count and progress updates normally; **two-source merge logic:** Phase 1 = DB query (authoritative CDN URLs), Phase 2 = storage walk (covers ~1,621 folders with no DB row), Phase 3 = merge (start with walk-derived map, overwrite with DB entries), Phase 4 = filter by localStorage compressed-paths set; **expected outcome after running v2:** ~2,425 files discovered (vs 804 previously), ~1,621 new files to compress at avg 1,353 KB → ~150 KB each, saving ~1.95 GB additional, dropping storage from ~3.1 GB to ~0.5–0.8 GB (under free tier 1 GB limit); **console logs to watch for:** `[Upload] recompressAll DB source | paths=804`, `[Upload] recompressAll storage walk | productFolders=2425`, `[Upload] recompressAll | total=~2425 needsWork=~1621 alreadyDone=804`; **⚠️ v2 deployed but NOT YET RUN — storage still at ~310% until user clicks the button**
