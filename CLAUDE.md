...
| `colorthief` | ^3.3.x | Pixel-based dominant color extraction. Used in `src/lib/colorUtils.ts` via `getPaletteSync` (sync browser API). |
...
- ✅ **Auto color detection via color-thief** (commit `b390208`) — when a product group first renders in Step 3 with an empty `color` field, a `useEffect` in `ComprehensiveProductForm` automatically samples all images in the group and writes the dominant color name (e.g. `"Black"`, `"Navy"`, `"Camel"`) to the `color` field; no button, no API call, entirely browser-side; implementation: `src/lib/colorUtils.ts` (new file) exports `extractGroupPrimaryColor(imageUrls[])` which loads each URL as a temp `Image` with `crossOrigin="anonymous"`, draws the center 50%×50% crop onto an off-screen canvas (original image untouched), runs `getPaletteSync(canvas, { colorCount: 5 })` from `colorthief` v3, filters neutral (near-white/near-black/grey) pixels that are likely background, clusters remaining RGB values by Euclidean proximity (radius 40), maps the largest cluster centroid to the nearest entry in `COLOR_RGB_MAP` (derived from `colorDatabase.ts`) via Euclidean nearest-neighbor, returns the color name string; multi-image groups: all images sampled in parallel, palette entries pooled before clustering; single-image groups: same code path, pool has 5 entries; secondary color intentionally **not** auto-filled — left for AI/voice/manual input; override priority: (1) manual typing, (2) AI/voice (writes unconditionally), (3) category preset that sets `color`, (4) color-thief auto (fires once on first render, guard `if (currentItem.color) return`); `colorthief` v3.3+ added as dependency (~10 KB)
- ✅ **Color database consolidated as single source of truth** (commit `c9594b8`) — `src/lib/colorDatabase.ts` (`COLOR_DNA`) is now the only place color names and hex codes are defined; two derived exports added at the bottom: `COLOR_RGB_MAP` (array of `{ name, rgb }` built by converting the first hex of each solid-color entry — used by `colorUtils.ts` for pixel nearest-neighbor lookup) and `COLOR_WORDS_LIST` (flat array of all canonical names + all aliases — used by `textAIService.ts` for voice/AI text scanning); the hardcoded 18-entry `COLOR_MAP` in `colorUtils.ts` and the hardcoded 31-word `COLOR_WORDS` array in `textAIService.ts` were both deleted and replaced with imports; pattern entries (`tie dye`, `camo`, `plaid`) have non-hex `hexCodes` values (`#MULTI` etc.) and are automatically excluded from both derived exports; multi-word aliases are sorted longest-first in text scanning so `"forest green"` matches before `"green"`; regex-escaped for special characters
- ✅ **COLOR_DNA expanded to 57 solid colors** (commits `152b9e1`, `cadfb35`) — added 26 new solid color entries covering common mass-produced clothing colors missing from the original 31: `black`, `red`, `blue`, `denim`, `cyan`, `green`, `yellow`, `orange`, `coral`, `salmon`, `maroon`, `pink`, `brown`, `beige`, `gray`, `white`, `ecru`, `light blue`, `sky blue`, `mint`, `lilac`, `mauve`, `peach`, `terracotta`, `taupe`, `camel`, `stone`; each entry includes hex codes, aliases, vibes, eras, subcultures, and commonIn fields; all new entries automatically flow into both `COLOR_RGB_MAP` (pixel detection) and `COLOR_WORDS_LIST` (voice/AI scanning) with no other file changes needed; to add a new color in future: add one entry to `COLOR_DNA` with a real hex code and it propagates everywhere automatically
- ✅ **Color-thief output restricted to 43 approved names** (commit `2363255`) — `ALLOWED_COLORS` set added to `colorUtils.ts`; `COLOR_MAP` (used for pixel nearest-neighbor lookup) is now filtered through this set before being used; color-thief can only write one of the 43 approved names into Step 3 `color` field; voice/AI scanning (`COLOR_WORDS_LIST`) is unaffected and still listens for all names and aliases in the full `COLOR_DNA`; to add/remove a Step 3 color, edit `ALLOWED_COLORS` in `colorUtils.ts`
...
- ✅ **Color-thief accuracy improvements** (commits `993c0cf`, `d9a3935`, `04df5fd`, `a5a42ab`, `71df4b9`) — series of improvements to `src/lib/colorUtils.ts`:
  - **Perceptually-weighted RGB distance** — green channel ×4, red ×2, blue ×1 (matches human color perception)
  - **Neutral filter tightened** — `s < 12` (was `s < 30`); old threshold wrongly discarded muted clothing colors like olive, sage, tan, beige, charcoal
  - **Cluster radius widened** — 55 (was 40); perceptually-similar shades (crimson/burgundy, navy/royal blue) now cluster together
  - **Palette count increased** — 8 colors per canvas (was 5)
  - **Multi-strip background filter** — `isLikelyBackground()` detects newspaper/parchment and wooden-lattice/ivy backdrops; filtered out when pool ≥ 6 entries to avoid discarding a tan garment
  - **Side-edge fabric zone sampling** (`getFabricZoneCanvases`) — 6 narrow zones (13% wide × 18% tall) along the LEFT and RIGHT edges of the image, 3 rows per side evenly distributed top-to-bottom (y: 12%–91%); left zones at x=10%, right zones at x=77%; deliberately avoids center where large prints/graphics live; works for ALL product types (tees, pants, hats, dresses, outerwear) not just hanging shirts — large center prints essentially never reach side edges
  - **Exposure normalization** (`normalizeExposure`) — each sampled RGB converted to HSB; hue preserved exactly; saturation clamped 20–95 (boosts washed-out faded fabric, caps oversaturated); brightness clamped 35–220 (lifts underexposed dark shots, pulls back overexposed blown-out areas); achromatic pixels (`s < 8`) passed through unchanged so true blacks/whites/greys are not falsely tinted; applied to every palette entry before adding to the pool
