...
│   │   ├── workflowStore.ts       # Dependency-free shared store (useSyncExternalStore, no library). Stage 2 foundation — getState() gives live reads in async code (replaces the ref-mirror pattern). NOT yet consumed by App/PDG; migration is one consumer per PR.
