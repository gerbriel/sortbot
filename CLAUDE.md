...
| `sortbot_analytics_session` (sessionStorage), `sortbot_analytics_force` (localStorage) | `analytics.ts` | Per-tab analytics session id (losing it just starts a new session) + the dev-only "track on localhost" override. Listed for the naming convention. |
...
**Tests:** Vitest (`npm test` to run once, `npm run test:watch` for watch mode). Config in `vitest.config.ts` (happy-dom environment; dummy Supabase env vars injected because `src/lib/supabase.ts` throws at import without them — tests never hit the network). Test files live next to their sources as `src/**/*.test.ts`. Current coverage (characterization tests — they lock in behavior the workflow depends on): `textAIService.test.ts` (size normalization incl. fits-like, voice command extraction, ✠ description lines, title length/letter-size/no-compounding rules, golden snapshot of a full description with `Math.random` mocked for determinism), `applyPresetToGroup.test.ts` (preset priority hierarchy: voice wins, preset fills gaps, force-mode semantics, SEO template interpolation), `workflowBatchService.test.ts` (deleted-batch tombstones + localStorage persistence + 200 cap), `analytics.test.ts` (tracker privacy contract: session id, DNT/localhost gating, referrer host only, row shape; dashboard math: funnel, compact numbers, deltas, tick ceilings), `crmService.test.ts` (tags, follow-up urgency, filter/sort/stage counts), `supportService.test.ts` (unread rules for both roles, thread ordering, relative timestamps). Snapshot lives in `src/lib/__snapshots__/` — update deliberately with `npx vitest run -u` only when output changes on purpose.
...
**Supabase Edge Function secrets** (server-side, set via `supabase secrets set`, NOT in `.env`): `SHOPIFY_STORE` and `SHOPIFY_ADMIN_TOKEN` — consumed by `supabase/functions/shopify-titles/index.ts` as the GLOBAL FALLBACK (Founding Workspace / legacy mode only). Per-org stores live in the `org_shopify_connections` table (July 2026): token is WRITE-ONLY for clients via column-level grants; only the Edge Function (service role) reads it. The Admin token must never appear in client code or `VITE_*` vars.
...
│   │   ├── SupportWidget.tsx      # Floating "Messages" button + panel (bottom-right, z 5000) for EVERY signed-in user incl. the waitlist gate; Founding admins get the inbox of all conversations (open/closed filter, reply, close/reopen). Realtime + 45 s poll. Hides itself pre-migration. Own CSS.
│   │   ├── AnalyticsPanel.tsx     # Founder tools → Analytics: 7/30/90-day range, KPI tiles with deltas, daily pageview column chart (plain HTML bars, per-bar tooltip, keyboard focusable), funnel + events/referrers/devices tables. Styles in OrgPanel.css (.an-*).
│   │   ├── CrmPanel.tsx           # Founder tools → CRM: stage chips, Due chip, search, inline stage/follow-up editing, expandable rows (name/company/tags, notes, delete), Add contact, Sync. Styles in OrgPanel.css (.crm-*).
...
│   │   ├── libraryData.ts         # PURE Library data derivation (extracted from Library.tsx loadAll): deriveLibraryData(wfBatches, savedProducts, savedImages) → {batches, groups, images}; owns ProductGroup/ImageRecord types, cleanTitle, makeBatchName, the two-pass imageList dedup/gap-fill rules, and batch synthesis. Tested in libraryData.test.ts.
│   │   ├── csvExport.ts           # PURE Shopify CSV generation (extracted from GoogleSheetExporter.tsx): taxonomy/type/GID maps, resolveCategoryPath/resolveProductType, buildCleanTitle, SHOPIFY_CSV_HEADERS (54 cols), buildShopifyCsv/buildShopifyCsvRows. Golden-tested in csvExport.test.ts.
│   │   ├── slimItems.ts           # slimForWorkflowState + ultraSlimForBackup (extracted from App.tsx autoSaveWorkflow). THE save→reload contract — any new ClothingItem field not recoverable from the DB must be added here or it vanishes on reload. Tested in slimItems.test.ts.
│   │   ├── workflowStore.ts       # Dependency-free shared store (useSyncExternalStore, no library). SOURCE OF TRUTH for the four item arrays since July 2026: App.tsx consumes it via useStoreItemArray (useState-compatible) + liveArrayRef views (replaced the ref mirrors). PDG still holds its duplicate processedItems copy — next migration step.
│   │   ├── orgService.ts          # Multi-org tenancy bootstrap (ensureOrganization with legacy fallback), members/invites CRUD. Includes the PRIVATE BETA GATE: new users without membership/invite get mode 'waitlist' unless their beta_signups row is approved; approved workspaces are created with plan='beta' and the requested shop name.
│   │   ├── imageRowSync.ts        # Stage 4 dual-write: stage4ColumnsAvailable() (cached probe — new columns are omitted until the migration runs, since one unknown column fails a whole upsert), buildTransforms(), buildProductImageRow() shared by registerItemsInDB + upload upsert + saveBatchToDatabase. Tested.
│   │   ├── betaService.ts         # Beta waitlist: getMyBetaSignup (own-row read, 'unavailable' fallback when migration not run), requestBetaAccess, fetchBetaSignups/setBetaStatus (Founding admins).
│   │   ├── analytics.ts           # FIRST-PARTY analytics: track()/trackPageview() insert rows into analytics_events (anon + authed), setAnalyticsContext (user/org ids), fetchAnalyticsSummary (analytics_summary RPC), pure helpers (shouldTrack: DNT + localhost, referrerHost, deviceClass, buildEvent, buildFunnel, compactNumber, percentDelta, niceCeiling). Auto-disables when the table is missing. Tested.
│   │   ├── crmService.ts          # FIRST-PARTY CRM (Founding admins): crm_contacts/crm_notes CRUD + syncCrmContacts() (crm_sync_contacts RPC: beta requests + accounts → contacts, never overwrites hand edits). Pure: filterContacts, sortContacts (follow-up urgency first), stageCounts, followUpStatus, parseTags, todayKey. 'unavailable' pre-migration. Tested.
│   │   ├── supportService.ts      # FIRST-PARTY messaging: support_threads/support_messages reads + writes (createThread, sendMessage, markThreadRead, setThreadStatus), subscribeToSupport (Realtime postgres_changes). Pure: isUnread(thread, role), unreadThreadCount, sortThreads, formatRelative. 'unavailable' pre-migration. Tested.
│   │   ├── foundingAdminService.ts  # Cross-workspace user management for Founding admins (OrgPanel "Users" tab). Wraps the founding_* SECURITY DEFINER RPCs: fetchAllUsers (every auth user + their memberships; [] pre-migration → tab hides), setMembership (add/change role), removeMembership, moveUser, fetchFoundingAudit. MEMBERSHIPS MOVE, DATA DOES NOT — batches/products keep their org_id.
│   │   ├── shopifyConnectionService.ts  # Per-org Shopify credentials. Token is WRITE-ONLY from the client (column-level grants — no SELECT on admin_token); getShopifyConnection reports 'unavailable' pre-migration so the UI hides. Save = delete+insert with return=minimal.
...
│       ├── beta_admin_directory.sql  # beta_org_directory() SECURITY DEFINER RPC: aggregate stats for ALL workspaces (name/plan/member emails/batch/product/image counts/last active) for Founding admins ONLY — no tenant data exposure, non-admins get zero rows. Run AFTER beta_signups.sql.
│       ├── founding_user_admin.sql   # ⚠️ PRIVILEGED (but additive): cross-workspace user management for Founding admins. founding_list_users / founding_set_membership / founding_remove_membership / founding_move_user (SECURITY DEFINER, gated on is_beta_admin()) + append-only founding_admin_audit table (authenticated has SELECT only; the functions write it as table owner). Memberships move; data does NOT (org_id untouched). Hard rail: no operation may leave the founding org with 0 owners+admins (raises + rolls back). SECURITY NOTE: unlike beta_org_directory, this necessarily lets a founding admin add THEMSELVES to any workspace and then read its data through normal org RLS — the audit log is the mitigation. Run AFTER multi_org_tenancy.sql + beta_signups.sql.
│       ├── stage4_slim_fields.sql    # Stage 4 dual-write columns: product_images.captured_at (bigint Unix ms) + original_storage_path, products.description_edited. Additive/idempotent. The app feature-detects these (imageRowSync probe) so code ships before the SQL runs.
│       ├── beta_signups.sql          # Private-beta waitlist table + RLS (anon INSERT pending-only; own-row SELECT; Founding-admin manage via is_beta_admin()). Run AFTER multi_org_tenancy.sql. Additive, idempotent, rollback included.
│       ├── org_shopify_connections.sql  # Per-org Shopify Admin credentials. admin_token is client-WRITE-ONLY (column grants); Edge Function reads it via service role. Run AFTER multi_org_tenancy.sql. Additive, idempotent, rollback included.
│       ├── analytics_events.sql      # FIRST-PARTY analytics (Sept 2026): analytics_events table (anon+authed INSERT with identity checks, SELECT is_beta_admin()), analytics_summary(days) + analytics_prune(days) SECURITY DEFINER RPCs. Additive, idempotent, rollback included. Run AFTER multi_org_tenancy + beta_signups.
│       ├── crm.sql                   # FIRST-PARTY CRM (Sept 2026): crm_contacts + crm_notes (all policies is_beta_admin(); column-level UPDATE grant on hand-edited columns only; note author pinned to caller), crm_sync_contacts() RPC reads beta_signups + auth.users + org_members. Additive, idempotent, rollback included.
│       ├── support_messaging.sql     # FIRST-PARTY messaging (Sept 2026): support_threads + support_messages, trigger keeps last_*/read stamps, RLS pins sender identity + role (user only in own thread, founder only if is_beta_admin()), replica identity full + supabase_realtime publication. Additive, idempotent, rollback included.
...
14. **Ref mirror pattern RETIRED (July 2026) — the names live on as store views.** `sortedImagesRef`, `groupedImagesRef`, `uploadedImagesRef`, and `processedItemsRef` are no longer per-render-updated `useRef` mirrors; they are module-level `liveArrayRef()` views into `workflowStore` whose `.current` getter ALWAYS reads the live store state. All ~30 existing `xxxRef.current` read sites work unchanged and can never be stale. The rule for new async handlers is unchanged in spirit: read `xxxRef.current` (or `workflowStore.getState()`), never the render-captured state variable. Historical context: the mirrors were added for the stale-closure bugs in `aae35fc`/`993c0cf`. One semantic difference from the mirror era: `.current` becomes fresh IMMEDIATELY after a setter call (mirrors only refreshed on the next render) — strictly closer to intent everywhere they're used.
...
17. **Do not put the Shopify Admin token anywhere in client code or `VITE_*` env vars.** It has full store read/write. It lives only as a Supabase Edge Function secret (`SHOPIFY_ADMIN_TOKEN`), consumed by `supabase/functions/shopify-titles/index.ts`.

18. **Do not replace the first-party analytics / CRM / messaging with a vendor, and do not add any tracking script, chat SDK, or external API call for them.** The user's rule is "100% self reliant" — these capabilities are features of this app on its own Supabase tables. Extend the tables, RPCs and React components instead. Also keep the three `'unavailable'` pre-migration paths intact (the UI hides itself until the SQL has run) and keep every read of these tables behind RLS as written: analytics SELECT and all CRM access are `is_beta_admin()`, threads are owner-or-founding, and `sender_role` must stay RLS-enforced.
