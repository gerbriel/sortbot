...
- ✅ **Auto color detection via color-thief** (commit `b390208`) — when a product group first renders in Step 3 with an empty `color` field, a `useEffect` in `ComprehensiveProductForm` automatically samples all images in the group and writes the dominant color name (e.g. `"Black"`, `"Navy"`, `"Burgundy"`) to the `color` field; no button, no API call, entirely browser-side; implementation: `src/lib/colorUtils.ts` (new file) exports `extractGroupPrimaryColor(imageUrls[])` which loads each URL as a temp `Image` with `crossOrigin="anonymous"`, draws the center 50%×50% crop onto an off-screen canvas (original image untouched), runs `getPaletteSync(canvas, { colorCount: 5 })` from `colorthief` v3, filters neutral (near-white/near-black/grey) pixels that are likely background, clusters remaining RGB values by Euclidean proximity (radius 40), maps the largest cluster centroid to the nearest entry in an 18-entry `COLOR_MAP` via Euclidean distance, returns the color name string; multi-image groups: all images are sampled in parallel and their palette entries pooled before clustering — a garment shot from 3 angles contributes ~15 palette entries, vastly outweighing background noise from any one image; single-image groups: same code path, pool has 5 entries; secondary color is intentionally **not** auto-filled — left for AI/voice/manual input; override priority: (1) manual typing, (2) AI/voice description (writes unconditionally to `color`), (3) category preset that sets `color`, (4) color-thief auto (fires once, guard `if (currentItem.color) return`); guard keyed on `currentItem.id` so it re-evaluates per group but never re-runs once the field has a value; `colorthief` v3.3+ added as a dependency (~10 KB)
